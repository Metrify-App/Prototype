    ...
    <Card className="overflow-hidden">
      <CardContent>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onTogglePin}
              title={
                isPinned ? 'Retirer de la sidebar' : 'Epingler dans la sidebar'
              }
              aria-label={
                isPinned
                  ? `Retirer ${project.name} de la sidebar`
                  : `Epingler ${project.name} dans la sidebar`
              }
        ...
        <div className="mt-4 grid grid-cols-4 gap-3">
          ...
          />
          <StatBadge icon={Eye} label="Views" value={project.stats.views} />
          <StatBadge
            icon={Users}
            label="Followers"
            value={project.stats.followers}
            highlight={
              project.stats.followers.startsWith('-') ? 'red' : 'green'
            }
          />
          <StatBadge
            icon={BarChart3}
        ...
        {project.videos.length > 0 && (
          <div className="mt-4 flex gap-3 overflow-x-auto pb-1">
            {project.videos.slice(0, 5).map((video) => (
              <div key={video.title} className="w-44 flex-shrink-0">
