...
export default function ProjectDetailPage({
  ...
  const viewsChartData = details.engagementOverTime.labels.map(
  ...
  const platformViewsData = details.platformStats.map((s: any) => ({
    ...
    views:
      parseInt(s.views.replace(/[^0-9]/g, '')) *
      (s.views.includes('M') ? 1000 : 1),
    fill: PLATFORM_COLORS[s.platform],
  }));

  const bestLogoPlatform = details.platformStats.reduce(
    (prev: any, curr: any) =>
      parseInt(curr.logoAppearance) > parseInt(prev.logoAppearance)
        ? curr
        : prev
  );
  const bestVocalPlatform = details.platformStats.reduce(
    (prev: any, curr: any) =>
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <p className="mt-1 text-sm text-[var(--text-muted)]">
        Core metrics regarding reach, engagement and media value.
      </p>
      ...
      <Card className="mt-6">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-base">
            All Content ({details.posts.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
                    ...
                    <TableCell className="text-right">
                      <span
                        className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${
                          parseInt(post.logoAppearance) >= 80
                            ? 'bg-[var(--color-green-light)] text-[var(--color-green)]'
                            : parseInt(post.logoAppearance) >= 60
      ...
      <p className="mt-1 text-sm text-[var(--text-muted)]">
        AI-powered analysis of brand logo exposure and verbal mentions.
      </p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="flex flex-col items-center justify-center gap-8 py-6">
            <LogoAppearanceRing
              percentage={parseInt(details.overview.logoAppearance)}
            />
          ...
          <CardContent className="pb-6 pt-2 flex flex-col items-center">
            <div className="h-[200px] w-full relative flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[9px] uppercase tracking-tighter text-[var(--text-muted)] font-semibold mt-0.5">
                  Mentions
                </span>
              </div>
            </div>
      ...
      {details.vocalClips && (
        <Card className="mt-6">
          <CardHeader>
            <CardTitle className="text-base">
              Speech Recognition Clips ({details.vocalClips.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  >
                    ...
                    <div className="flex gap-3 px-1">
                      ...
                      <div className="flex flex-col min-w-0">
                        <h4 className="text-sm font-bold leading-tight line-clamp-2 mb-1">
                          {clip.title}
                        </h4>
      ...
      {details.brandMentions && (
        <>
          ...
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            Audience sentiment and specific mentions in comments and chat.
          </p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {details.brandMentions.overview.topKeywords.map(
                    (kw: string) => (
                      <span
                        key={kw}
          ...
          <Card className="mt-6">
            <CardHeader>
              <CardTitle className="text-base">
                Brand Mentions ({details.brandMentions.comments.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-0 divide-y divide-[var(--border-light)]">
                {details.brandMentions.comments.map((comment: any) => {
                  ...
                  const sentiment =
                    SENTIMENT_CONFIG[
                      comment.sentiment as keyof typeof SENTIMENT_CONFIG
                    ];
